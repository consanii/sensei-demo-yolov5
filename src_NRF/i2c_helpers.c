...
int32_t st_i2c_write_reg(void *handle, uint8_t reg, const uint8_t *bufp, uint16_t len) {
...
int32_t st_i2c_read_reg(void *handle, uint8_t reg, uint8_t *bufp, uint16_t len) {
  i2c_ctx_t *ctx = (i2c_ctx_t *)handle;
...
int8_t bosch_i2c_write_reg(uint8_t reg_addr, const uint8_t *reg_data, uint32_t length, void *intf_ptr) {
  i2c_ctx_t *ctx = (i2c_ctx_t *)intf_ptr;
  LOG_DBG("[0x%02X] Address 0x%02X:", ctx->i2c_addr, reg_addr);
  LOG_HEXDUMP_DBG(reg_data, length, "I2C TX");
  return i2c_burst_write(ctx->i2c_handle, ctx->i2c_addr, reg_addr, reg_data, length);
}

int8_t bosch_i2c_read_reg(uint8_t reg_addr, uint8_t *reg_data, uint32_t length, void *intf_ptr) {
  i2c_ctx_t *ctx = (i2c_ctx_t *)intf_ptr;
  int error = i2c_burst_read(ctx->i2c_handle, ctx->i2c_addr, reg_addr, reg_data, length);
  LOG_DBG("[0x%02X] Address 0x%02X:", ctx->i2c_addr, reg_addr);
  LOG_HEXDUMP_DBG(reg_data, length, "I2C RX");
  return error;
}

void bosch_i2c_sleep_usec(uint32_t period, void *intf_ptr) {
  int32_t remaining = period;
  while (remaining > 0) {
    remaining = k_usleep(remaining);
  }
}

int8_t sensirion_i2c_read_reg(uint8_t address, uint8_t *data, uint16_t count) {
  return i2c_read(i2c_b, data, count, address);
}

int8_t sensirion_i2c_write_reg(uint8_t address, const uint8_t *data, uint16_t count) {
  return i2c_write(i2c_b, data, count, address);
}

void sensirion_i2c_sleep_usec(uint32_t useconds) {
