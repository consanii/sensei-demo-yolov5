...
#define GPIO_NODE_gap9_shared_ctrl DT_NODELABEL(gpio_gap9_shared_ctrl)
...
static const struct gpio_dt_spec gpio_p0_20_gap9_spi_ctrl = GPIO_DT_SPEC_GET(GPIO_NODE_gap9_spi_ctrl, gpios);
static const struct gpio_dt_spec gpio_p0_28_gap9_shared_ctrl = GPIO_DT_SPEC_GET(GPIO_NODE_gap9_shared_ctrl, gpios);
...
int pwr_bsp_init() {
  ...
  // Configure GAP9 Shared GPIO control pin
  if (gpio_pin_configure_dt(&gpio_p0_28_gap9_shared_ctrl, GPIO_OUTPUT_INACTIVE) < 0) {
    LOG_ERR("GAP9 SHARED GPIO init error");
    return 0;
  }
...
int pwr_bsp_start() {
  ...
  pmic_conf->sbb_conf[0].en = MAX77654_REG_OFF;
  ...
  pmic_conf->ldo_conf[0].en = MAX77654_REG_OFF;
  ...
  // Enable shared GPIOs (also required for nRF <-> GAP UART)
  if (gpio_pin_set_dt(&gpio_p0_28_gap9_shared_ctrl, 1) < 0) {
    LOG_ERR("GAP9 Shared GPIO configuration error");
    return 0;
  }
  LOG_INF("GAP9 Shared GPIOs enabled");
  ...
  if (gpio_pin_set_dt(&gpio_p0_20_gap9_spi_ctrl, 1) < 0) {
    LOG_ERR("GAP9 SPI GPIO configuration error");
    return 0;
  } else {
    LOG_INF("GAP9 connected to SPI bus");
  }
